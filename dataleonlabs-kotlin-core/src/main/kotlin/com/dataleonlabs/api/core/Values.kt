...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString() ?: throw DataleonlabsInvalidDataException("Value is not a string")
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw DataleonlabsInvalidDataException("`$name` is not set")
            is JsonNull -> throw DataleonlabsInvalidDataException("`$name` is null")
            else -> throw DataleonlabsInvalidDataException("`$name` is invalid, received $this")
        }
